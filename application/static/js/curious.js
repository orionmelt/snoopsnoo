!function(){
  ...
  curious.timeseries = function(options) {
    ...
    if(!data.length || !("date" in data[0])) {
      console.log("Error: Data doesn't contain field named 'date'.");
    }
    ...
    var ys_axis_fn = 
      (secondary_scale && secondary_scale.length>0) ? 
      (d3.svg.axis().scale(ys).orient("right").tickFormat(d3.format(".2s"))) : 
      null;
    ...
    y_max = d3.max(
      series_values.filter(function(d) { return secondary_scale.indexOf(d.name)<0;}),
      function(d) { return d3.max(d.values, function(v) { return v.value; }); }
    );
    ys_max = d3.max(
      series_values.filter(function(d) { return secondary_scale.indexOf(d.name)>-1;}),
      function(d) { return d3.max(d.values, function(v) { return v.value; }); }
    );

    y_min = d3.min(
      series_values.filter(function(d) { return secondary_scale.indexOf(d.name)<0;}),
      function(d) { return d3.min(d.values, function(v) { return v.value; }); }
    );
    ys_min = d3.min(
      series_values.filter(function(d) { return secondary_scale.indexOf(d.name)>-1;}),
      function(d) { return d3.min(d.values, function(v) { return v.value; }); }
    ...
    if(tooltips) {
      ...
      var tip = d3.tip()
        ...
        .offset([-10, 0])
        .html(function(d) {
          return  "<p>" +  d3.time.format(tooltip_date_format)(d.date) +
                  "</p>" + d3.format(",d")(d.value) + " " +
                  this.parentNode.__data__.name.replace("_"," ");
        });
      ...
      .enter().append('circle')
        ...
        .attr("cy", function(d) { 
          return secondary_scale.indexOf(this.parentNode.__data__.name)<0 ? y(d.value) : ys(d.value); 
        })
        .attr("r", 4)
  ...
  curious.bar = function(options) {
    ...
    y_max = d3.max(
      data,
      function(d) {
        return d3.max(d.series_values, function(d) { if(secondary_scale.indexOf(d.name)<0) return d.value; });
      }
    );
    ys_max = d3.max(
      data,
      function(d) {
        return d3.max(d.series_values, function(d) { if(secondary_scale.indexOf(d.name)>-1) return d.value; });
      }
    );
    ...
    if(tooltips) {
      ...
      var tip = d3.tip()
        ...
        .html(function(d) {
          return  "<p>" + 
                  (tooltip_names ? tooltip_names[this.parentNode.__data__[x_col]] : this.parentNode.__data__[x_col]) +
                  "</p>" + d3.format(",d")(d.value) + " " + d.name;
        });
  ...
  curious.heatmap = function(options) {
    ...
    var height = options.height;
    var margin = options.margin;
    var color = options.color || d3.scale.linear().domain([0,15]).range(["#ffeeee","#FF0000"]);
    var tooltips = options.tooltips || false;
